...
describe("parse", function() {
  ...
  it("can parse a floating point number without integer", function() {
    var fn = parse('.42');
    expect(fn()).toBe(0.42);
  });
  ...
  it("can parse a scientific notation with + sign", function() {
    var fn = parse('.42e+2');
    expect(fn()).toBe(42);
  });
  ...
  it("can parse s string with single quotes inside", function() {
    var fn = parse("'a\\\'b'");
    expect(fn()).toEqual('a\'b');
  });

  it("can parse s string with double quotes inside", function() {
    var fn = parse("'a\\\"b'");
    expect(fn()).toEqual('a\"b');
  });

  it("will parse a stirn gwith unicode escapes", function() {
    var fn = parse('"\\u00A0"');
    expect(fn()).toEqual('\u00A0');
  });
  ...
  it("looks up an attribute from scope", function () {
    var fn = parse('aKey');
  ...
  it('parse a simple computed property access', function() {
    var fn = parse('aKey["anotherKey"]');
    expect(fn({aKey: {anotherKey: 42}})).toBe(42);
  });
  ...
  it('parse a function call with multiple arguments', function() {
    var fn = parse('aFunction(36, n, argFn())');
    expect(fn({
      n: 3,
      argFn: _.constant(2),
  ...
  it('does not allow window as computed property', function() {
    var fn = parse('anObject["wnd"]');
    expect(function() {fn ({anObject : {wnd: window}}); }).toThrow();
  });

  it('does not allow window as non-computed property', function() {
    var fn = parse('anObject.wnd');
    expect(function() {fn ({anObject : {wnd: window}}); }).toThrow();
  });
