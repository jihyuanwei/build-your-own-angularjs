...
describe('$http', function () {
  ...
  it('resolve promise when XHR result received', function() {
    var requestConfig = {
      method: 'GET',
      url: 'http://teropa.info'
    };
  ...
  it('sets header on request', function() {
    $http({
      url: 'http://teropa.info',
      headers: {
        'Accept': 'text/plain',
  ...
  //POST method have body, need content-type
  ...
  it('supports function as header value', function() {
  ...
  it('allows setting default response transform', function () {
  ...
  it('transform error responses also', function() {
    var response;
    $http({
      url: 'http://teropa.info',
      transformResponse: function(data) {
  ...
  it('parse JSON data for JSON responses', function() {
    var response;
    $http({
      method: 'GET',
      url: 'http://teropa.info'
  ...
  it('does not choke on response resembling JSON but invalid', function() {
    var response;
    $http({
      method: 'GET',
      url: 'http://teropa.info'
  ...
  it('allows subsituting param serializer', function() {
    $http({
      url: 'http://teropa.info',
      params: {
        a: 42,
    ...
    expect(requests[0].url)
      .toBe('http://teropa.info?a=42lol&b=43lol');
  });
  ...
  it('makes default param serializer available throught DI', function() {
    var injector = createInjector(['ng']);
  ...
  describe('JQ-liek param serialization', function() {
  ...
  it('supports shorthand method for GET', function() {
    ...
    $rootScope.$apply();
    expect(requests[0].url).toEqual('http://teropa.info?q=42');
    expect(requests[0].method).toBe('GET');
  });
  ...
  it('allows attaching success handlers', function() {
    ...
    requests[0].respond(200, {'Cache-Control': 'no-cahce'}, 'Hello');
    $rootScope.$apply();

    expect(data).toBe('Hello');
    expect(status).toBe(200);
    expect(headers('Cache-Control')).toBe('no-cahce');
  ...
  it('allows attaching error handlers', function() {
    ...
    requests[0].respond(401, {'Cache-Control': 'no-cahce'}, 'Fail');
    $rootScope.$apply();
  ...
  describe('useApplyAsync', function() {
    ...
    it('does not resolve promise immediately when enabled', function() {
      ...
      $http.get('http://teropa.info').then(resolveSpy);
      $rootScope.$apply();
